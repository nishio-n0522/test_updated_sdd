...
**1ファイルあたり300行以下を推奨、400行を上限とする。** 超える場合はセクション分割（ディレクトリ化）する。共通コンポーネントは専用ファイルに定義し、各ドキュメントからは参照のみ行う。同じ定義を複数ファイルにコピーしない。
...
- **data-model/** - データモデル設計書（index.md + 層/ドメイン別ファイル。エンティティのフィールド定義・型・制約・ER図）
