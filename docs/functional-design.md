...
**概要**: 標準的な検査シナリオのテンプレートを提供し、画像処理の専門知識が少ないユーザーでも検査フロー構築を始められるようにする。テンプレートを起点にフローをカスタマイズして使用する。

**含まれるサブ機能**:
- テンプレート一覧: 用途・検査種別で分類されたテンプレートの一覧表示と選択
- テンプレートからのフロー作成: テンプレートを元に新規フローを作成する
...
- テンプレートから作成したフローはプロジェクトに紐づけて保存できる
...
| SCR-004 | プロジェクト一覧画面 | プロジェクトの一覧表示と作成・選択 | FN-003 | MVP |
| SCR-005 | フローエディタ画面 | 検査パイプラインをノードベースで設計するメイン画面 | FN-001, FN-002, FN-007, FN-009 | MVP |
...
**構成エリア**:
- ヘッダー: ナビゲーション
- プロジェクト作成エリア: 新規プロジェクトの作成を開始する起点
...
sequenceDiagram
    ...
    User->>App: フローエディタ画面でテンプレート選択を開く
    App-->>User: テンプレート一覧（用途別分類）を表示
    User->>App: 用途に近いテンプレートを選択
    App-->>User: テンプレートの説明・カスタマイズポイントを表示
    User->>App: テンプレートからフロー作成を実行
    App-->>User: テンプレートのノード構成がキャンバスに展開される
...
1. フローエディタ画面からテンプレート一覧を開く
2. 検査目的に近いテンプレートを選択し、説明を確認する
3. テンプレートからフローを作成するとキャンバスにノード構成が展開される
...
| テンプレート機能（Phase 4） | FN-009 / SCR-005 |
