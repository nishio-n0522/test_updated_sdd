...
| SCR-004 | プロジェクト一覧画面 | メイン   | プロジェクトの一覧表示と作成・選択。作成時にテンプレート選択も可能     | FN-003, FN-009     | MVP         | `scr-004-project-list.md`  |
| SCR-005 | フローエディタ画面 | メイン    | 検査パイプラインをノードベースで設計するメイン画面                     | FN-001, FN-002, FN-007 | MVP    | `scr-005-flow-editor.md`   |
...
| グローバルナビゲーション     | 全画面共通でサイドバー形式のグローバルナビゲーションを表示する。SCR-002〜SCR-009では開状態で常時表示し、どの画面からでも主要エリアへ直接アクセスできる。ホーム画面（SCR-001）ではサイドバーは閉状態（アイコンのみ）で表示し、ホバー時にオーバーレイで展開する |
