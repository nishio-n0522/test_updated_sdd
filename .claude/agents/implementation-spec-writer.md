...
8. **記述レベル（責務境界）チェック**を実施する:
   - `docs/architecture/` にコード例（関数定義、フック実装、型定義、コンポーネント等）が含まれていないか → 含まれていれば削除し、該当する design-patterns に移動
   - `docs/architecture/` にテーブルスキーマ・SQL定義が含まれていないか → data-model の責務
   - `docs/development-guidelines/` に完全な実装パターン（15行超のコードブロック）が含まれていないか → 含まれていれば短いスニペットに縮小し、design-patterns への参照を追加
   - `docs/repository-structure/` にコード例や実装パターンが含まれていないか
   - 同一のコードブロックが複数ドキュメントに記載されていないか → 1箇所に集約し、他は参照に変更
9. 不整合や責務逸脱が見つかった場合、該当するドキュメントを直接修正する
