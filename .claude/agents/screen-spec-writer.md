...
3. グローバル `shared-components.md` を作成する（複数のSCR-xxxをまたいで共通のUIコンポーネント）
   - グローバル共通コンポーネント（サイドバー、ヘッダー、トースト通知等）のUI要素定義と共通操作を定義
...
5. 各画面ファイルを作成する（**1画面ずつ、ユーザー承認を得てから次に進む**）
   ...
   - **Step 5b: 画面仕様作成**
     ...
     - グローバル共通コンポーネントは `../shared-components.md` への参照のみ記載し、重複させない
     - 画面ディレクトリ内で複数ビューに共通するコンポーネント（ドロワー、ビュー切替等）がある場合は、`scr-xxx-yyy/shared-components.md` を作成して定義する（グローバルCOMP番号とは別体系）
...
- 共通コンポーネントはグローバル（`shared-components.md`）と画面内（`scr-xxx/shared-components.md`）の2層で管理し、各画面に重複定義しない
