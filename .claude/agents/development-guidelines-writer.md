...
開発ガイドラインは**コーディング規約**のレベルで記述する。

- **コード例は短いスニペット（5-15行）に限定**: 規約を示すための最小限の例のみ。完全な実装例（フック全体、クラス全体等）は design-patterns の責務
- **技術選定理由は書かない**: architecture の責務
- **ディレクトリ構造は書かない**: repository-structure の責務
