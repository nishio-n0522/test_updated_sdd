...
- `prompt`: "今回実装した `[機能名]` に関する全ての変更点がプロジェクトのデザインパターンに準拠しているか検証してください。対象ファイルは `[実装したファイルのパスリスト]` です。フィーチャーベースアーキテクチャ、データフェッチング3層パターン、状態管理パターン、コンポーネント構成ルール、インポートパス・モジュール境界ルール、命名規則を重点的に確認してください。"
...
- ステップ6: `implementation-validator`および`design-pattern-checker`サブエージェントの検証をパスする（[必須]レベルの問題がないこと）。
