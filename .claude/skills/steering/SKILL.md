...
1. **ステアリングディレクトリの確認**
   ```
   現在の日時を取得し、`.steering/[YYYYMMDDHHmmss]-[機能名]/` の形式でディレクトリを作成
   ```
...
2. **永続ドキュメントの確認**
   ...
   - `docs/design-patterns/` - デザインパターン（技術スタック別のコーディングパターン）
   - `docs/screen-specification/` - 画面仕様書（存在する場合）
...
3. **テンプレートからファイル作成**
   ...
   - `.claude/skills/steering/templates/requirements.md` → `.steering/[日時]-[機能名]/requirements.md`
   - `.claude/skills/steering/templates/design.md` → `.steering/[日時]-[機能名]/design.md`
   - `.claude/skills/steering/templates/tasklist.md` → `.steering/[日時]-[機能名]/tasklist.md`
...
```
Read('.steering/[日時]-[機能名]/tasklist.md')
```
