...
**注意**: ここでのUI設計は**概要レベル**で十分です。「だいたいこんなアプリ」という全体像を示すことが目的です。各画面の全UI要素の定義、状態別ワイヤーフレーム、操作ごとの詳細挙動は、実装が進んだ後に `screen-specification` スキルで定義します。

**概要レベルで記載すべき内容**:
- 画面一覧（どんな画面があるか）
- 代表的な1状態のレイアウト（大まかな要素配置）
- 画面遷移の全体像
- カラーコーディング等のデザインルール
